...
const inputClass =
  'min-h-12 w-full rounded-lg border border-border-light bg-white py-3 pl-12 pr-4 text-sm transition focus:border-accent-gold focus:outline-none focus:ring-2 focus:ring-accent-gold/25';
...
export default function SignupForm() {
  ...
  const handleStep1 = (e: React.FormEvent) => {
    ...
    // 1. Check empty
    if (!trimmedFullName || !trimmedUsername || !trimmedEmail || !trimmedPhone) {
      setError(localize("Vui lòng điền đầy đủ các trường bắt buộc.", ERROR_REQUIRED));
      return;
    }

    // 2. Full Name (2-50 chars, letters only)
    ...
    if (!nameRegex.test(trimmedFullName)) {
      setError(localize("Họ và tên phải có từ 2 đến 50 ký tự và chỉ gồm chữ cái.", ERROR_INVALID_FULL_NAME));
      return;
    }

    // 3. Email format check (thay thế check domain cứng)
  ...
  const handleStep2 = async (e: React.FormEvent) => {
    ...
    if (!password || !confirmPassword) {
      setError(localize("Vui lòng nhập và xác nhận mật khẩu.", ERROR_REQUIRED));
      return;
    }

    // 2. Check Match First (Ưu tiên hiển thị thông báo mật khẩu không trùng khớp)
    if (password !== confirmPassword) {
      setError(localize("Mật khẩu xác nhận không khớp.", ERROR_PASSWORD_MATCH));
      return;
    }

    // 3. Check spaces
    if (/\s/.test(password)) {
      setError(localize("Mật khẩu không được chứa khoảng trắng.", ERROR_PASSWORD_SPACE));
      return;
    }
    ...
    if (!strictPasswordRegex.test(password)) {
      setError(localize("Mật khẩu cần ít nhất 8 ký tự, gồm chữ hoa, chữ thường, số và ký tự đặc biệt.", ERROR_PASSWORD_STRICT));
      return;
    }
    ...
    <>
      ...
      {step === 1 && (
        <>
          <div className="mb-7">
            ...
            <h1 className="font-serif text-4xl font-semibold tracking-tight text-[#0F2A43]">{localize("Tạo tài khoản khách hàng", "Create your guest account")}</h1>
            <p className="mt-3 text-sm leading-6 text-[#66727C]">{localize("Đặt phòng nhanh hơn và theo dõi toàn bộ kỳ nghỉ trong một nơi.", "Book faster and track your entire stay in one place.")}</p>
          </div>
          <form onSubmit={handleStep1} className="space-y-5">
            ...
            <div className="mb-4">
              <FormInput id="signup-fullname" label={localize("Họ và tên", LABEL_FULL_NAME)} required icon={<UserIcon />}>
                <input
                  id="signup-fullname"
                  type="text"
                  autoComplete="name"
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <FormInput id="signup-username" label={localize("Tên đăng nhập", LABEL_USERNAME)} required icon={<UserIcon />}>
                <input
                  id="signup-username"
                  type="text"
              ...
              <FormInput id="signup-email" label={localize("Email", LABEL_EMAIL)} required icon={<MailIcon />}>
                <input
                  id="signup-email"
                  type="email"
                  autoComplete="email"
            ...
            <FormInput id="signup-phone" label={localize("Số điện thoại", "Phone number")} required icon={<PhoneIcon />}>
              <input
                id="signup-phone"
                type="tel" 
                autoComplete="tel"
            ...
            <button
              type="submit"
              className="mt-2 flex min-h-12 w-full items-center justify-center gap-2 rounded-lg bg-[#0F2A43] px-4 py-3.5 font-semibold text-white transition hover:bg-[#091E30] active:translate-y-px focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#B8944F] focus-visible:ring-offset-2"
            >
      ...
      {step === 2 && (
        <form onSubmit={handleStep2} className="space-y-5">
          <div className="mb-2">
            <p className="mb-2 text-xs font-bold uppercase tracking-[0.2em] text-[#80632F]">{localize("Bước 2 / 2", "Step 2 / 2")}</p>
            <h2 className="font-serif text-3xl font-semibold text-[#0F2A43]">{localize("Thiết lập mật khẩu", "Set your password")}</h2>
            <p className="mt-2 text-sm leading-6 text-text-light">{localize("Chỉ còn một bước. Hãy tạo mật khẩu an toàn cho tài khoản.", "Almost done. Create a secure password for your account.")}</p>
          </div>
          ...
          <FormInput id="signup-password" label={localize("Mật khẩu", LABEL_PASSWORD)} required icon={<LockIcon />}>
            <input
              id="signup-password"
          ...
          <FormInput id="signup-confirm" label={localize("Xác nhận mật khẩu", LABEL_CONFIRM_PASSWORD)} required icon={<LockIcon />}>
            <input
              id="signup-confirm"
          ...
          <div className="flex gap-3">
            <button
              ...
              className="flex min-h-12 flex-1 items-center justify-center gap-2 rounded-lg border border-border-light px-4 py-3.5 font-semibold text-text-dark transition hover:bg-white"
            >
              <ArrowLeftIcon />
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="flex min-h-12 flex-1 items-center justify-center gap-2 rounded-lg bg-[#0F2A43] px-4 py-3.5 font-semibold text-white transition hover:bg-[#091E30] disabled:cursor-not-allowed disabled:opacity-60"
            >
              {isLoading ? localize("Đang tạo tài khoản...", BTN_REGISTERING) : (
                <>
