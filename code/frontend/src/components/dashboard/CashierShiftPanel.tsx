...
export default function CashierShiftPanel({ embedded = false, adminView = false }: { embedded?: boolean; adminView?: boolean }) {
  ...
  const netCashMovement = current ? expected - Number(current.openingCashAmount || 0) : 0;
  const openingSuggestion = useMemo(
    () => suggestedOpeningCash(history.content, !adminView),
    [adminView, history.content],
  );
  ...
  const openModal = async (kind: Exclude<ModalKind, null>) => {
    ...
    if (kind === "close" && current) {
      ...
      setAmount(kind === "open"
        ? String(openingSuggestion.amount)
        : "");
    }
    setModal(kind);
  };
  ...
  return (
    <section className={embedded ? "w-full" : "mx-auto w-full max-w-[1500px] px-4 py-6 sm:px-6 lg:px-8"}>
      <header className={`flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between ${embedded ? "pb-5" : "border-b border-[#0F2A43]/12 pb-6"}`}>
        <div><p className="text-[11px] font-extrabold uppercase tracking-[0.2em] text-[#9A762F]">{adminView ? "Kiểm soát tiền mặt · ADMIN" : "Vận hành tiền mặt · STAFF"}</p><h1 className="mt-2 font-serif text-3xl font-bold text-[#0F2A43] sm:text-4xl">{adminView ? "Ca thu ngân & sổ tiền mặt" : "Ca thu ngân"}</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-[#66727C]">{adminView ? "ADMIN xem được lịch sử mọi ca; nếu trực tiếp thu tiền, ADMIN vẫn chỉ mở và thao tác ca của chính mình." : "Thanh toán và hoàn tiền CASH được tự ghi vào ca. Nhân viên chỉ xác nhận tiền thật đầu ca và kiểm đếm cuối ca."}</p>{lastUpdatedAt && <p className="mt-2 text-[11px] font-semibold text-[#7A858E]">Tự cập nhật mỗi 30 giây · gần nhất {lastUpdatedAt.toLocaleTimeString("vi-VN", { hour: "2-digit", minute: "2-digit", second: "2-digit" })}</p>}</div>
      ...
      {loading ? <div className="mt-6 grid gap-4 lg:grid-cols-3" aria-label="Đang tải ca thu ngân">{[1, 2, 3].map((item) => <div key={item} className="h-36 animate-pulse rounded-2xl bg-[#0F2A43]/7" />)}</div> : current ? (
        <section className="mt-6 overflow-hidden rounded-2xl border border-[#0F2A43]/12 bg-white shadow-[0_16px_45px_rgba(15,42,67,0.08)]">
          ...
          <div className="grid gap-px bg-[#0F2A43]/8 sm:grid-cols-2 xl:grid-cols-4">{[["Tiền thật đầu ca", formatVnd(current.openingCashAmount)], ["Phát sinh tiền mặt ròng", `${netCashMovement >= 0 ? "+" : "−"}${formatVnd(Math.abs(netCashMovement))}`], ["Tiền cuối ca hệ thống tính", formatVnd(current.expectedCashAmount)], ["Số phát sinh", `${current.movementCount} giao dịch`]].map(([label, value]) => <div key={label} className="bg-white px-5 py-5 sm:px-6"><p className="text-[10px] font-extrabold uppercase tracking-[0.16em] text-[#7A858E]">{label}</p><p className="mt-2 font-serif text-2xl font-bold text-[#0F2A43]">{value}</p></div>)}</div>
      ...
      ) : <section className="mt-6 rounded-2xl border border-dashed border-[#0F2A43]/20 bg-[#F8F6F0] px-6 py-12 text-center"><div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-[#E8DFC9] text-2xl" aria-hidden="true">₫</div><h2 className="mt-4 font-serif text-2xl font-bold text-[#0F2A43]">Chưa có ca thu ngân đang mở</h2><p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-[#66727C]">Nếu két bắt đầu rỗng, giữ số 0 ₫ đã được điền sẵn. Chỉ sửa khi thực tế có tiền bàn giao đầu ca.</p><button type="button" onClick={() => void openModal("open")} className="mt-5 min-h-11 rounded-lg bg-[#0F2A43] px-6 text-sm font-bold text-white transition hover:bg-[#173D5F]">Mở ca ngay</button></section>}
      ...
      <ViewportModal open={Boolean(modal)} onClose={closeModal} labelledBy="cashier-action-title" busy={submitting} panelClassName="max-w-lg">
        ...
        <div className="lux-scrollbar min-h-0 overflow-y-auto px-5 py-5">
          {modal === "close" && <div className="mb-4 rounded-xl bg-[#F1F0EA] p-4 text-sm"><div className="grid grid-cols-2 gap-3"><div><p className="text-xs text-[#66727C]">Tiền cuối ca hệ thống tính</p><p className="mt-1 font-bold text-[#0F2A43]">{formatVnd(expected)}</p></div><div><p className="text-xs text-[#66727C]">Chênh lệch kiểm đếm</p><p className={`mt-1 font-bold ${closeVariance === 0 ? "text-emerald-700" : "text-rose-700"}`}>{closeVariance > 0 ? "+" : ""}{formatVnd(closeVariance)}</p></div></div><p className="mt-3 border-t border-[#0F2A43]/10 pt-3 text-xs leading-5 text-[#66727C]">Hệ thống đã điền số dự kiến. Hãy thay bằng số tiền mặt bạn thực sự đếm được; chỉ khi hai số khác nhau mới cần giải trình.</p></div>}
          ...
          {modal === "open" && <p className="mt-2 text-xs leading-5 text-[#66727C]">{openingSuggestion.source
            ? `Đã gợi ý theo tiền kiểm đếm cuối ca gần nhất (${openingSuggestion.source.shiftCode}). Hãy xác nhận lại số tiền thật đang có trong két.`
            : "Hệ thống điền 0 ₫ khi chưa có ca trước. Hãy thay đổi nếu két đang có tiền bàn giao."}</p>}
